(function main() {
  ...
  const initialDimensions = ['Maternal mortality ratio per 100,000 live births', 'Mobile-cellular subscriptions per 100 inhabitants', 'Tuberculosis death rate per year per 100,000 population (mid-point)', 'Seats held by women in national parliament, %', 'Children 1 year old immunized against measles, %', 'Population undernourished, %'];
  ...
  const bundleEdges = (nodes, edges, index, callback) => {
    if (window.Worker) {
      // Use web worker to perform edge bundling
      const worker = new Worker('worker.js');
      worker.postMessage({ nodes: nodes, edges: edges });
      worker.onmessage = e => callback(e.data, index);
    } else {
      const bundlingResults = d3.ForceEdgeBundling()
        .nodes(nodes)
        .edges(edges)
        .step_size(0.02)
        .iterations(30)();
  ...
  // Perform edge bundling on all sections of the parallel
  // coordinates and organize results in meaningfull way.
  const prepareLines = function prepareLines(allData, dimensions, callback) {
    ...
    allData.forEach(d => lines.push(dimensions.map(p => [x(p), ranges[p](d[p])])));

    lines.forEach(() => bundledLines.push(new Array(dimensions.length - 1)));
    ...
    lines[0].forEach((_, i) => {
      ...
      lines.forEach((localLine, j) => {
        nodes[`${j}_1`] = { x: localLine[i][0], y: localLine[i][1] };
        nodes[`${j}_2`] = { x: localLine[i + 1][0], y: localLine[i + 1][1] };
        edges.push({ source: `${j}_1`, target: `${j}_2` });
      });

      // Perform edge bundling
      bundleEdges(nodes, edges, i, (bundlingResults, index) => {
        bundlingResults.forEach(function forEachBundlingResult(oneLine, j) {
          bundledLines[j][index] = oneLine.map(point => [point.x, point.y]);
        });

        count += 1;

        if (count === (dimensions.length - 1)) {
          bundledLines = bundledLines.map(oneLine => oneLine.reduce((a, b) => a.concat(b), []));
  ...
  const drawChart = function drawChart(allData, dimensions) {
    if (allData === undefined || allData.length === 0 || !dimensions) {
      return;
    }
    ...
    function brush() {
      const actives = dimensions.filter(p => !ranges[p].brush.empty());
      const extents = actives.map(p => ranges[p].brush.extent());
      foreground.classed('displayed', d => actives.every((p, i) => extents[i][0] <= d[p] && d[p] <= extents[i][1]));
    }

    // Extract the list of dimensions and create a scale for each.
    x.domain(dimensions);

    prepareLines(allData, dimensions, lines => {
      // Add lines.
      foreground = svg.append('g')
        ...
        .enter()
        .append('path')
        .attr('d', (_, i) => line(lines[i]))
        .on('mouseover', function foregroundMouseOver(d) {
          d3.select('#text').text(d['']);
          this.parentNode.appendChild(this);
        })
        .on('mouseout', () => d3.select('#text').text(''));

      // Entry animation effect
      foreground.each(function animateForeground() {
        const oneLine = d3.select(this);
        setTimeout(() => oneLine.classed('displayed', true), Math.random() * 2500);
      });
    });
    ...
    g.append('g')
      .attr('class', 'axis')
      .each(function createAxis(d) {
        d3.select(this).call(axis.scale(ranges[d]).ticks(3));
      })
      .append('text')
      .style('text-anchor', 'middle')
      .attr('y', (_, i) => (i % 2 === 0 ? -12 : -32))
      .text(d => d);

    // Remove tick lines
    d3.selectAll('.tick line').remove();
    d3.selectAll('.axis path.domain')
      .attr('d', function removeTickLines() {
    ...
    g.append('g')
      .attr('class', 'brush')
      .each(function createBrush(d) {
        d3.select(this).call(ranges[d].brush = d3.svg.brush().y(ranges[d]).on('brush', brush));
      })
      .selectAll('rect')
      .attr('x', -8)
      .attr('width', 16);
  ...
  d3.selectAll('#top, #goback-btn')
    .on('click', () => d3.selectAll('#add, #about').classed('active', false));
  ...
  goal.append('span')
    .classed('icon', true)
    .on('click', function iconClick() {
      d3.select(this)
        .classed('expanded', !d3.select(this).classed('expanded'));
      d3.select(this.parentElement)
        .select('span.goal')
        .classed('active', !d3.select(this.parentElement).select('span.goal').classed('active'));
    });

  goal.append('span')
    .classed('goal', true)
    .on('click', function spanClick() {
      d3.select(this)
        .classed('active', !d3.select(this).classed('active'));
      d3.select(this.parentElement)
        .select('span.icon')
        .classed('expanded', !d3.select(this.parentElement).select('span.icon').classed('expanded'));
    })
    .text(d => d);
  ...
  indicators
    ...
    .attr('value', d => d)
    .attr('name', 'indicator')
    .attr('checked', d => (initialDimensions.indexOf(d) > -1 ? 'checked' : null));
  ...
  d3.select('#update-chart-btn')
    .on('click', () => {
      const dimensions = d3.selectAll('input:checked').data().sort();
      const filteredData = data.filter(datum => dimensions.every(dim => datum[dim] !== ''));

      d3.selectAll('#add, #about').classed('active', false);
      setTimeout(() => drawChart(filteredData, dimensions), 100);
    });

  // LOAD DATA
  d3.tsv('parallel_2013.csv', (error, csv) => {
    // Assign data to the variable in the outer scope
    data = csv;

    // Populate indicators totals
    indicators.selectAll('span.total')
      .text(indicator => data.filter(country => country[indicator] !== '').length)
      .attr('title', function title() {
        return `There are ${d3.select(this).text()} countries in this dataset containing this indicator`;
      });

    // Click on 'Update chart' button
    d3.select('#update-chart-btn').on('click')();
  });
}());
