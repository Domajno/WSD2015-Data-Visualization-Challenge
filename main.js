...
var drawChart = function(data, dimensions) {
	...
	if(data === undefined || data.length === 0 || !dimensions) return;
	...
	for (var i = 1; i<lines[0].length; i++) {
		...
		var fbundling = d3.ForceEdgeBundling().nodes(nodes).edges(edges).step_size(0.02).iterations(30);
...
d3.tsv("parallel_2013.csv", function(error, csv) {
	...
	.attr('title', function(){ return 'There are ' + d3.select(this).text() + ' countries in this dataset containing this indicator';});	
...
// Top buttons interactions

d3.selectAll('#top, #goback-btn').on('click', function () {
	d3.selectAll('#add, #about').classed('active', false);
});

d3.select('#about-btn').on('click', function () {
	d3.event.stopPropagation();
	d3.select('#add').classed('active', false);
...
d3.select('#add-btn').on('click', function () {
	...
	d3.select('#add').classed('active', !d3.select('#add').classed('active'));
});

// Populate checkboxes
var goal = d3.select('#add ul').selectAll('li').data(d3.keys(goals)).enter().append('li');
...
.on('click', function () {
	...
	d3.select(this.parentElement).select('span.goal').classed('active', !d3.select(this.parentElement).select('span.goal').classed('active'));
});
...
.on('click', function () {
	...
	d3.select(this.parentElement).select('span.icon').classed('expanded', !d3.select(this.parentElement).select('span.icon').classed('expanded'));
})
.text(function(d){ return d; });
...
.selectAll('ul')
.selectAll('li')
.data(function(d){ return goals[d].sort();})
.enter()
.append('li');
...
.attr('checked', function(d){ 
	return dimensions.indexOf(d) > -1 ? 'checked' : null;
});
...
d3.select('#update-chart-btn').on('click', function(){
	var dimensions=d3.selectAll('input:checked').data().sort();
	var filteredData = data.filter(function(datum){
	    return dimensions.every(function(dim){
	        return datum[dim] !== '';
	    });
	...
	d3.selectAll('#add, #about').classed('active', false);
	setTimeout(function(){drawChart(filteredData, dimensions);}, 100);
});